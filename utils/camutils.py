...
def cam_to_roi_mask2(cam, cls_label, hig_thre=None, low_thre=None):
    ...
    # print(f'cam_value: {cam_value.shape}')      # 2*448*448
    # _pseudo_label += 1
...
def crop_from_roi_neg(images, roi_mask=None, crop_num=8, crop_size=96):
    ...
    for i1 in range(b):
        ...
        for i2 in range(crop_num):
            ...
            # print(f'temp_crops: {temp_crops.shape}')  # 2*8*3*96*96
            ...
            if temp_mask.sum() / (crop_size*crop_size) <= 0.2:
                ...
                flags[i1, i2+2] = 0     # +2 是因为函数外要在local_crops前加上两个pos crops
    
    _crops = torch.chunk(temp_crops, chunks=crop_num, dim=1,)
    # print(f'_crops.shape: {_crops[0].shape}')  2*1*3*96*96
    # print(f'_crops.shape: {len(_crops)}')  8
    crops = [c[:, 0] for c in _crops]
    # print(f'crops.len: {len(crops)}')  8
...
def multi_scale_cam2(model, inputs, scales):
    ...
    with torch.no_grad():
        ...
        for s in scales:
            if s != 1.0:
                # 等价于仿射变换，把原图放大缩小，获得cam后再放缩回原图大小
        ...
        # print(f'stackcam_list.shape: {torch.stack(cam_list, dim=0).shape}')  # [3, 2, 20, 448, 448]
        cam = torch.sum(torch.stack(cam_list, dim=0), dim=0)
        # print(f'cam.shape: {cam.shape}')        # 2*20*448*448
        cam = cam + F.adaptive_max_pool2d(-cam, (1, 1))     # 减自适应最小
        cam /= F.adaptive_max_pool2d(cam, (1, 1)) + 1e-5    # 除自适应最大

        cam_aux = torch.sum(torch.stack(cam_aux_list, dim=0), dim=0)
        cam_aux = cam_aux + F.adaptive_max_pool2d(-cam_aux, (1, 1))
        cam_aux /= F.adaptive_max_pool2d(cam_aux, (1, 1)) + 1e-5    # 辅助cam 归一化

    return cam, cam_aux


def label_to_aff_mask(cam_label, ignore_index=255):
    # cam_label: 2*28*28 标签
    b, h, w = cam_label.shape   # b: 2    h,w = 28
    ...
    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)   # 转置相同的说明是正对 设置为True（有些是255，也被设置为正对）
    
    for i in range(b):
        aff_label[i, :, _cam_label_rep[i, 0, :] == ignore_index] = ignore_index     # 消除255被设置为的正对
        aff_label[i, _cam_label_rep[i, 0, :] == ignore_index, :] = ignore_index
    aff_label[:, range(h*w), range(h*w)] = ignore_index  # 忽略对角线
    return aff_label
