...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image, local_image 一样，只是random裁剪不一样
...
class VOC12_Visual_Dataset(VOC12Dataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
    ...
    def __transforms(self, image):
        ...
        return image, local_image, img_box, crop_ori_image
    ...
    def __getitem__(self, idx):
        ...
        ori_image_t = torch.tensor(image)
        ori_image_t_n = self.normalize(image)

        pil_image = Image.fromarray(image)
        # 原图

        image, local_image, img_box, crop_ori_image = self.__transforms(image=image)
        # image, local_image 一样，只是random裁剪不一样
        ...
        if self.aug:
            ...
            return img_name, image, cls_label, img_box, crops, crop_ori_image, ori_image_t, ori_image_t_n
        else:
            return img_name, image, cls_label
